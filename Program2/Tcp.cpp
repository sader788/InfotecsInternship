...
int TCPListener::receive(uint8_t *data, size_t length) {
    if (!connected) {
        return -1;
    }

    int bytesNum = read(clientSocketDescriptor, data, length);
...
int TCPListener::send(const uint8_t *data, size_t length) {
    if (!connected) {
        return -1;
    }

    int bytesNum = write(clientSocketDescriptor, &data, length);
...
bool TCPSender::isInitialized() const {
    return initialized;
}
